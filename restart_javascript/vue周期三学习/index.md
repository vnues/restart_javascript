...
先子后父  那是因为父级创建完后也需要被挂载



computed出来的目的就是解决模板臃肿问题 当我们的数据依赖于数据去计算 这时候就是需要computed属性 其实这时候尽量用它，因为有缓存作用 提高性能❗️
...
通过这一节对 nextTick 的分析，并结合上一节的 setter 分析，我们了解到数据的变化到 DOM 的重新渲染是一个异步过程

所以宏任务和微任务确实理解起来确实很重要❗️
...
注意了 不管是初始化还是更新 我们生成的dom都是根据render函数来的  这一点认知很重要 就比如我们手动去创建挂载dom，但是此时数据发生变化 更新了 不是在在这个dom进行再一次更新

反正dom的生成是根据render函数来的 昨天遇到字体高亮 脑袋不知道为啥这样想
...
vm.$nextTick就是把你的回调函数 注册到微任务中去❗️❗️❗️
而如果使用vm.$nextTick向微任务队列中插入任务，则代码中注册的顺序就非常重要，因为渲染操作和使用vm.$nextTick注册的回调都是向微任务队列中添加任务


❗️❗️❗️我们在项目开发中会遇到这样一种场景：当我们将状态改变之后想获取更新后的DOM，往往我们获取到的DOM是更新前的旧DOM

我们在methods方法中修改了数据以后想要拿到更新后的dom 虽然这里vue提供生命周期updated但是如果在那里拿到dom也行 enenenn....但是位置总感觉飘的有点远

首先vue更新dom是异步的  我们只要在宏任务或者微任务中注册任务（顺序要求）就行、
所以为啥用setTimeout 0能拿到的原因 因为宏任务慢执行于微任务
...
为什么要使用使用异步来更新dom呢，那是因为假设我这时刻同时数据改变了10次那么前面9次显然很没有意义 有点像防抖的味道
enenen...这也是一部分
...
```
就像上面所说的一样 我们做个防抖不就行了吗  但是你要想想防抖作用的行为不就是异步行为 这也是一点


另外一点  用异步的话我们好取最后一次操作  这是重点❗️❗️❗️
...
> 那如何才能将渲染操作推迟到所有状态都修改完毕呢

很简单，只需要将渲染操作推迟到本轮事件循环的最后或者下一轮事件循环(这样一想 是不是很容易拿到最后一次操作))。也就是说，只需要在本轮事件循环的最后，等前面更新状态的语句都执行完之后，执行一次渲染操作，它就可以无视前面各种更新状态的语法，无论前面写了多少条更新状态的语句，只在最后渲染一次就可以了。

更新dom还不是走update方法 假设就是update方法
...
所以说vue的dom更新是异步操作这点认知很重要  异步操作涉及的知识点就是js的运行机制 也就是事件循环  就是一个知识点 不难的
...
下次DOM更新循环结束=====>只有下次DOM更新完才会触发  你就理解触发DOM更新的时候如果有这个函数就会触发但是跟触发的注册顺序有关
...
```
官方对nextTick做了降级处理 如果不支持promise会使用setTimeout宏任务代替 宏任务的执行顺序是等待微任务执行完才会去执行
...
setTimeout 0为什么不会马上执行 事件循环的知识  这句话你可以这样理解 过了0s把回调函数添加到宏任务中去




以前项目的竹林表单日历中 操作数据后拿不到dom用setTimeout来拿 以前debug是因为知道更新后可能先执行了拿到的是旧dom所以自己自然而然用setTimeout

但是官方提供了nextTick来解决这个问题  在下次 DOM 更新循环结束之后执行延迟回调。在修改数据之后立即使用这个方法，获取更新后的 DOM❗️❗️❗️

❗️❗️❗️但是这不是重点 重点为什么会用异步去拿 就是vue dom更新是异步操作  所以得利用js事件机制去判断回调执行顺序 放在合适的执行顺序去执行

还有自己写异步操作时候应该想到Js执行机制 事件循环 用事件循环的原理去判断❗️❗️❗️这才是重要的


重新学习了下事件循环 确实简单很多了 就是一个知识点
...
浏览器发出请求
服务器监听到 80 端口（或443）有请求过来，并解析url路径
...
首先我们运行项目就是起的脚手架 当我们输入localhost:8080 实际就是向后端请求这个api
...
但是如果我们改变的hash值是不会刷新浏览器的
然后hash一改变我们就去监控它 返回不同的dom（页面）不就达到效果了吗
...
还有一点重要的是 这种hash值的变化不会向服务器发送请求

如果是非前端路由且存在这个api就会向后端发起请求
...
hash模式下 假设这个链接是http://www.xxx.com/#/login
刷新后会向服务器先请求http://www.xxx.com/请求

vue再根据hash后的值渲染login页面


2. history 模式

14年后，因为 HTML5 标准发布。多了两个 API，pushState 和 replaceState，通过这两个 API 可以改变 url 地址且不会发送请求。同时还有popstate 事件。通过这些就能用另一种方式来实现前端路由了，但原理都是跟 hash 实现相同的。用了 HTML5 的实现，单页路由的 url 就不会多出一个#，变得更加美观。但因为没有 # 号，所以当用户刷新页面之类的操作时，浏览器还是会给服务器发送请求。`为了避免出现这种情况，所以这个实现需要服务器的支持，需要把所有路由都重定向到根页面。` 难怪啊 需要服务器的支持

我就说前端路由关后端毛事 原来是这道理

还有hash路由还有锚点的作用 这是浏览器的默认行为（超哥说的）

### Vue router 实现



那么如果push到首页的时候是不是还会去请求一次 我觉得会
...
实现思路🤔：current.route内存放当前路由的配置信息，所以我们只需要监听current.route的变化来动态render页面便可。
...
```javascript
  ...
  // watcher就是依赖  这里我们添加一个观察者  只要route数据改变 watcher能观察到 通知render更新

```
恩....到这里，我们似乎已经完成了一个简单的响应式数据更新。其实render也就是动态的为页面指定区域渲染对应内容，这里只做一个简化版的render:


🤔：vue的实现路由的方法 就是把路由当做一个路由对象 并且还是响应路由对象  这样想也对
...
那么我有一个问题我们是怎么改变路由对象的 ❗️ 首先是数据驱动，所以我们可以通过一个route对象来表述当前路由状态，比如：

（非刷新情况下）也就是当前路由变化了 路由对象也就改变了 也就触发了
...
还有也就是路由对象的依赖就是render watcher



所以说前端路由跟后端路由区别很大

但是做多页面 我们可以Node服务自己做代理转发


话说是怎么改变路由的

总不可能我们在地址哪里改变后面的hash再输入enter键就会刷新了
不是这样的


就是我们在vue哪里执行this.$router.push操作应该是改变的是路由对象 路由对象一改变 render执行更新变化

然后页面一变化

页面变化时，我们只需要监听hashchange和popstate事件，做路由转换transitionTo
...
`hash（#）是URL 的锚点，代表的是网页中的一个位置，单单改变#后的部分（/#/..），浏览器只会加载相应位置的内容(这个加载相应的内容是vue-router来操作的)，不会重新加载网页`，也就是说 #是用来指导浏览器动作的，对服务器端完全无用，HTTP请求中不包括#；同时每一次改变#后的部分，都会在浏览器的访问历史中增加一个记录，使用”后退”按钮，就可以回到上一个位置；所以说Hash模式通过锚点值的改变，根据不同的值，渲染指定DOM位置的不同数据
...
所以公司可以使用history模式  只要node层支持就行。用了 HTML5 的实现，单页路由的 url 就不会多出一个#，变得更加美观。但因为没有 # 号，所以当用户刷新页面之类的操作时，浏览器还是会给服务器发送请求。`为了避免出现这种情况，所以这个实现需要服务器的支持，需要把所有路由都重定向到根页面。


window.location.hash="1232"

原来是这样啊

![](./2.png)

我们通过执行window.location.hash改变了hash值此时改变了route对象（响应式对象） 

此时this.$router.push我估计就是最终走这步（如果是hash模式的话）window.location.hash="1232"

`hash的改变会自动添加到浏览器的访问历史记录中`⚠️。 那么视图的更新是怎么实现的呢，看下 transitionTo()方法：

history.transitionTo 是 Vue-Router 中非常重要的方法，当我们切换路由线路的时候，就会执行到该方法

所以说transitionTo是来更新dom?
...
所以transitionTo是History模式下的？

而且看出，当传入一个新的路由的时候会transitionTo()会进行如下处理:
1.根据传入路由与当前路由进行对比，并返回相应的路由对象
2.判断新路由是否有效，并分别调用对应的回调函数 3.若有效，则调用成功回掉，结合初始化中

原来这样啊

http://binzhome.com/2018/03/13/about-vue-router/
...
相当于transitionTo做一层拦截操作

明天再来做下梳理总结

hashHistory与HTMLhistory
...
最终实现一个miniVue就行了 还要输出博文

国庆7天需要加班加点写出博文 把上面👆两个周期学习的东西输出,感觉到时候还要重新看一遍

要总结归纳消化成自己的

html css还需要开半个周期时间系统性梳理一遍



我先打一波理论基础再来debuggger和手动实现

不然容易陷进去

有了理论基础 我才知道自己要干嘛

看全局管理我觉得不应该看vuex应该看redux


现在的安排这一个月的周期完成博客和miniVue的实现 书籍还没到 所以先写博文吧
就这样交叉就行
...
然后就是设计模式只是一种模式   并没有数据结构和算法重要

就算我不知道这个东西 写着写着我也能总结出来


数据结构和算法、设计模式他们都讲究理论和思想 解决问题的方法  而不是api调用
